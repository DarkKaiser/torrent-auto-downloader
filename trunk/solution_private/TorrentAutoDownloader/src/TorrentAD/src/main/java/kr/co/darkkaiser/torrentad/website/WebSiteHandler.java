...
public interface WebSiteHandler {
	 ...
	 * 
	 * @param board 검색할 게시판
	 * @param loadAlways 검색할 게시판의 게시물을 다시 조회할지의 여부, false일 경우 기존에 읽어들인 데이터가 있을 경우 기존 데이터를 반환한다.
	 */
	 ...
	 * @param loadAlways 등록된 게시판의 게시물을 다시 조회할지의 여부, false일 경우 기존에 읽어들인 데이터가 있을 경우 기존 데이터를 반환한다.
	 */
	...
	/**
	 * 모든 게시판에서 해당 키워드를 포함하는 게시물을 항상 다시 검색한다.
	 * 
	 * @param keyword 키워드
	 */
	Iterator<WebSiteBoardItem> searchAllBoards(String keyword);
