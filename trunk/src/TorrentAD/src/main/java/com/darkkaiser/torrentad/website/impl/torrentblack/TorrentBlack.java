...
public class TorrentBlack extends AbstractWebSite {
	...
	public static final String BASE_URL = "https://www.torrentblack.com";
	...
	private static final String FILETENDER_DOWNLOAD_URL = "https://file.filetender.com/file2.php";

	public TorrentBlack(final WebSiteConnector siteConnector, final String owner, final String downloadFileWriteLocation) {
		super(siteConnector, owner, WebSite.TORRENTBLACK, downloadFileWriteLocation);
	}
	...
	public Tuple<String/* 검색기록 Identifier */, Iterator<WebSiteBoardItem>/* 검색결과목록 */> search(final WebSiteBoard board, final String keyword, final Comparator<? super WebSiteBoardItem> comparator) throws NoPermissionException, LoadBoardItemsException{
		...
		List<WebSiteBoardItem> boardItems = loadBoardItems0_0(board, String.format("&sca=&sop=and&sfl=wr_subject&stx=%s", keyword));
		if (boardItems == null)
	...
	protected List<WebSiteBoardItem> loadBoardItems0_0(final WebSiteBoard board, final String queryString) throws NoPermissionException {
		...
		try {
			final TorrentBlackBoard siteBoard = (TorrentBlackBoard) board;
				...
				if (elements.isEmpty() == true) {
					...
					try {
						for (final Element element : elements) {
							...
							if (detailPageURL.startsWith(String.format("%s/board.php", TorrentBlack.BASE_URL_WITH_DEFAULT_PATH)) == false)
	...
	protected Tuple<Integer/* 다운로드시도횟수 */, Integer/* 다운로드성공횟수 */> downloadBoardItemDownloadLink0(final WebSiteBoardItem boardItem) {
		...
		while (iterator.hasNext() == true) {
			TorrentBlackBoardItemDownloadLink downloadLink = (TorrentBlackBoardItemDownloadLink) iterator.next();
	...
	public String toString() {
		return TorrentBlack.class.getSimpleName() +
                "{" +
                "}, " +
                super.toString();
	}
